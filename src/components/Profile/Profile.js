...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8080/api/getUser`, config).then(res => setProfile(res.data)).catch(err => console.error(err));
  }, [])
  ...
  const updateProfile = (e) => {
    ...
    axios.put(`http://localhost:8080/api/updateUser`, values, config).then(res => window.location.reload(true)).catch(err => console.error(err));
  }
              ...
              <Form onSubmit={updateProfile}>
                <CardBody>
                  ...
                  <Row className={'viewHosRow'}>
                    <Col sm={3}>Email</Col>
                    <Col sm={1}>:</Col>
                    <Col sm={4}><Input type="email" name="email" value={values.email || profile.email} disabled="disabled" onChange={handleChange}/></Col>
                  </Row>
                  <Row className={'viewHosRow'}>
